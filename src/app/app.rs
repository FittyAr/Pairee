    ...
    loop {
        ...
        if state.update_progress_rx.is_some() {
            ...
            if let Some(p) = latest_progress {
                ...
                state.update_status = crate::update::UpdateStatus::Downloading(p);
            }
        ...
        if state.update_install_rx.is_some() {
            ...
            match rx.try_recv() {
                Ok(result) => {
                    ...
                    match result {
                        Ok(crate::update::installer::InstallResult::RestartRequired) => {
                            state.update_status = crate::update::UpdateStatus::Done;
                            state.active_popup = Some(PopupType::Info(
                        ...
                        Err(err) => {
                            state.update_status = crate::update::UpdateStatus::Error(err.clone());
                ...
                Err(tokio::sync::oneshot::error::TryRecvError::Empty) => {
                    ...
                    if state.update_progress_rx.is_none() && state.update_status != crate::update::UpdateStatus::Installing {
                        state.update_status = crate::update::UpdateStatus::Installing;
                    }
                }
                Err(tokio::sync::oneshot::error::TryRecvError::Closed) => {
                    // Task died/panicked
                    state.update_progress_rx = None;
                    state.update_status = crate::update::UpdateStatus::Error("Installation task terminated unexpectedly".to_string());
                    if let Some(PopupType::UpdateAvailable { error, install_progress, .. }) =
                        &mut state.active_popup
                    {
