//! M2 typed userdata: `Url` and `Path`.
//!
//! Both types wrap a `PathBuf` plus an optional `Scheme` so plugins
//! can manipulate local and SFTP URLs uniformly:
//!
...
//!   `is_absolute`, `has_root`. The same fields are available on
//!   `Path` (which only ever represents local filesystem paths).
//! - Methods: `join`, `starts_with`, `ends_with`, `strip_prefix`,
//!   `into_search`.
//! - Metamethods: `__eq`, `__tostring`, `__concat`.
//!
//! The implementation deliberately keeps the type simple — the
//! richness of "what can I do with this Url" comes from the methods
//! on `Cha` / `File` (M2-T3/T4) and from the new `fs.*` operations
//! in M3-T6.
...
/// The M2 `Path` userdata — same shape as `Url` but always local.
#[derive(Debug, Clone)]
pub struct PathU {
    pub path: PathBuf,
}
...
mod tests {
    ...
    fn test_path_lua_basics() {
        ...
        lua.globals().set("p", lua.create_userdata(p).unwrap()).unwrap();
