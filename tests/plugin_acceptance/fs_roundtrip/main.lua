...
function M.run(root)
    ...
    assert(f.mime == "text/plain", "File.mime")
    assert(f.is_hidden == false, "File.is_hidden")
    assert(type(f.is_exec) == "boolean", "File.is_exec")
    assert(type(f.mtime) == "number", "File.mtime")
    assert(tostring(f):find("a.txt", 1, true), "File tostring")

    pairee.fs.write(join(dir, ".secret"), "x")
    local hidden = pairee.fs.file(join(dir, ".secret"))
    assert(hidden.is_hidden == true, "dotfile is_hidden")

    local listed = pairee.fs.read_dir(dir)
    assert(#listed == 3, "read_dir count")
