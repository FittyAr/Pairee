...
- **End-to-end retry-as-admin (Sub-project 2, B1–B7 of `implementation_plan_2.md`).** A new `TransferPolicy` trait in `fs::transfer::policy.rs` lets the engine consult pluggable behaviour on every file failure. The default engine uses a `PromptPolicy` that accumulates `AccessDenied` failures and emits a single `TransferEvent::PermissionPrompt` at the end of the job; a new 3-button `PopupType::PermissionPrompt` (Yes / No / Cancel) lets the user accept the retry; the background loop translates the answer into `FsOperation`s and hands them to `run_in_elevated_helper` (UAC on Windows, `sudo` on Unix). `FsOperation` now covers `Rename`, `CreateLink { kind }` and `SecureErase` so every operation the engine performs is also retryable as admin. SSH `AccessDenied` is downgraded to `FileError::IoError` and excluded from the prompt because the helper runs locally and cannot escalate the remote server's permissions. `TransferEvent::PermissionDenied` is emitted per file for UI feedback. 15 new tests cover the trait, the policy batching, the SSH downgrade, the popup handler, the retry derivation, and the event plumbing.
- **Compress / Extract on the unified engine (Sub-project 1, A1–A10 of `implementation_plan_2.md`).** New `ArchiveFormat` (Zip / TarGz / SevenZ) and two new `TransferOperation` variants (`Compress { format, level }`, `Extract { format }`) flow through the same `TransferEndpoint`-aware path as Copy / Move / Delete. `compress_pipeline` writes the archive to a `Vec<u8>` first (the format crates demand `Read + Seek`) and flushes to the destination endpoint; `extract_pipeline` is the mirror and rejects `..`, absolute paths and NUL bytes per entry. 7Z encoding runs inside `tokio::task::spawn_blocking` because the `sevenz-rust` API is synchronous. The legacy `fs/ops_worker/{compress,extract}.rs`, the `spawn_compress_task` / `spawn_extract_task` re-exports, `state.progress_rx`, the `app::app::background::block 1` progress drain and `fs/apply_cmd.rs` are all gone. The Compress and Extract popups now build `TransferJob`s and submit them through `state.transfer.engine`; the archive browser still calls the legacy `fs::archive::list_archive_files` helper (kept trimmed to detect_format + list_archive_files; the new pipeline is write-only, so the UI's "what's inside" listing is the only piece still on the old code path). 12 new tests cover the round-trip, level-0 ZIP, cancellation, multi-format compress, all three extract formats, and the path-traversal sanitiser.
...
- Secure wipe routed through the unified engine (Phase 9). New `TransferOptions::wipe_passes` field (clamped to 3) plus a `secure_wipe` helper on the worker that overwrites the file with alternating byte patterns before the unlink. The Wipe action and the `WipeConfirm` popup now enqueue a `TransferJob::Delete` with `wipe_passes = 3`. Wipe is silently skipped on SSH (SFTP cannot guarantee overwrite semantics on remote files). The `fs::wipe` and `fs::ops_worker::wipe` modules are gone.
- Atomic cleanup of the legacy file-operation modals (Phase 10). `BackgroundOpContext` and `state.active_bg_op` are gone (the engine has its own state). The `fs::ops_worker::{copy_move, delete, helper, wipe}` files, the `fs::wipe` and `fs::link` modules, `SharedSshClient::{create_dir, walk_dir, rename_move, delete_recursive}`, the `spawn_copy_move_task` / `spawn_ssh_delete_task` / `spawn_wipe_task` re-exports and the `copy_extended_attributes`-style option flag plumbing in `actions/fs_ops/{copy,move}.rs` are all deleted. `state.progress_rx` is kept (it now feeds only the legacy compress / extract path through `ops_worker`, which is the only piece outside the unified engine). `PopupType::CopyProgress` is also kept for that same path; the engine has its own `TransferPanel` for copy / move / delete / rename / create-link / wipe.
...
- **Plugin menu actions no longer leak `println!` text into the TUI.** Every TUI plugin action (uninstall, toggle trust / pin, update, update-all) now reports its outcome through the non-modal toast channel, while the underlying `updater` functions are silent and return structured results. The CLI commands in `main.rs` keep their human-readable output by formatting the structured result themselves. The `update()` function now returns a per-plugin `UpdateReport` (instead of `()`), so the `U` action in the Installed tab emits one toast per plugin plus a summary; the user no longer has to guess which plugin failed. 3 new dispatcher tests cover the new `PluginActionStarted` / `PluginActionFinished` request types and the per-popup action lock, mirroring the `InstallFinished` pattern from the previous fix.
- **Plugin Search tab now shows install / update state per entry.** Each row in the registry search list is badged with a `✓` (already installed at the latest version, dim green) or a `↑` (installed but a newer version is available in the registry, bright yellow). The detail panel shows the locally installed version next to the registry version. The starting toast before the HTTP round-trip uses the right verb — `Installing X…` for a fresh install, `Reinstalling X…` for an exact-version reinstall, `Updating X…` when a newer version is available — so the user always knows what `i` is about to do. A new `app::input_popup::plugin_menu::search::install_status` helper centralises the `(name, registry_version) -> {NotInstalled, Installed, UpdateAvailable}` decision; 4 new tests cover it.
- **Plugin Search tab no longer spawns parallel installs.** Pressing `i` while an install is in flight now shows a 2-second warn toast ("Install in progress — please wait…") and the second task is dropped. The per-popup `PluginMenu.install_in_progress` flag is set by the search handler and cleared by a new `PluginRequest::InstallFinished { name }` the install task sends when it returns (success or failure). The dispatcher only clears the flag when the name matches, so a stale finish for an old install cannot clobber a newer one. The `println!` calls inside `updater::install` were removed (they corrupted the TUI in raw mode); progress is now reported exclusively through toasts. The CLI `pairee plugin update` path keeps its own progress via `update_all()`. 3 new dispatcher tests cover the lock release, the name-mismatch guard, and the no-popup-active no-op.
- **Keybinding resolver rewritten with explicit source attribution and conflict detection.** The previous design stored `key -> Action` and let the preset win outright; a plugin asking for `F2` while the preset had `user_menu` on `F2` silently lost, and the user had no way to find out. The new resolver keeps a `key -> ResolvedBinding { action, source, plugin_action }` map where `source` is one of `Builtin` (preset), `User` (`keybindings.toml`), `Plugin { plugin }` (manifest), or `Lua { plugin }` (runtime). A fixed priority table decides the winner when two sources register the same key: `User (100) > Lua (80) > Plugin (60) > Builtin (40)`. User bindings are permanent — no policy can displace them. The plugin registry's `register_plugin` now calls `resolver.register` for every `[keybindings]` entry, and a conflict is logged with the full attribution (`"plugin 'disk-usage.pairee': keybinding 'Ctrl+R' -> 'entry' refused (key already owned by builtin)"`) so the user can fix the manifest or rebind the preset. New `PluginRequest::PluginCommand` sentinel action; the dispatch loop in `app::app::events` is a single `match` that runs `handle_action` for `Builtin` / `User` and forwards to `plugin::registry::run_command` for `Plugin` / `Lua`. The resolver's static layers (preset + user) are immutable; plugin / Lua layers live in a `RwLock` overlay that the plugin loader and the Lua API mutate at runtime, so the read path on every keypress is one `try_read` then a `HashMap` lookup. The `AppContext::resolver` field is now `Arc<KeybindingResolver>` so the plugin VM's Lua closures can hold a `Send + Sync` handle to the same instance the main loop reads. 12 new resolver tests + 5 source tests + 3 Lua API tests cover the priority table, the conflict policies (`Fallback` / `Override`), the unregister path, and the user-takes-precedence guarantee.
- **Plugins can now claim a key at runtime through `pairee.keybindings.bind`.** A new `pairee.keybindings` Lua module exposes `bind(key, action, opts?)`, `unbind(key)` (still a stub — returns a "not implemented" error so the Lua-side surface is symmetric), `unbind_plugin()` (releases every key the calling plugin owns; the per-key form lands in a follow-up), and `list()` (a snapshot of every binding currently in the resolver, with `key` / `plugin_action` / `source` columns). The `opts` table accepts a `mode` field: `"fallback"` (default — only claim free keys), `"steal"` / `"override"` (force-claim and replace the current owner), and `"fail"` / `"fail_if_occupied"` (strict — reject the binding unless the key is currently unbound). The bind call returns `(ok, err_or_nil)` so a plugin can branch on the outcome. Registrations flow through the same resolver the manifest entries use, so the priority + conflict rules are identical. Two new locales strings document the new "Plugins" sub-tab in the plugin menu (under construction — the surface is wired but the diagnostics UI itself is a follow-up).
- **Three preset keys freed for plugins.** `User Menu` moved from `F2` to `Alt+F2`; `Refresh` moved from `Ctrl+R` to `Ctrl+Shift+R`; `Drive Select Right` moved from `Alt+F2` to `Alt+Shift+F2` to keep both reachable. The change ships in `keymaps/norton.toml`, `keymaps/vscode.toml`, and `keymaps/neovim.toml` (where `Refresh` moved to `F5` instead, after dropping the duplicate `copy_fkey = "F5"` entry — `copy = "y"` still works). The built-in fallback in `src/keybindings/preset.rs` matches the file presets. With these moves, the three FittyAr plugins that the user installed (`archive-inspect.pairee` claims `F2`, `disk-usage.pairee` claims `Ctrl+D`, `recent-files.pairee` claims `Ctrl+R`) now bind without conflicts on the default Norton preset.
...
- **Security review (branch `refactor-plugins`): elevation helper, transfer policy, archive sanitiser, open_writer, mode preservation, code-quality cleanups.** Eleven findings, eleven fixes:
  - **C1 — `run_in_elevated_helper` temp file is now created with a random name and mode 0600 on Unix.** The previous implementation wrote the ops JSON to `/tmp/pairee_op_<pid>.json` via `std::fs::write`, which honours the process umask (mode 0644 = world-readable) and used a PID-predictable filename. A concurrent local user could (a) read the file and learn which paths the user was about to elevate, and (b) replace the file with a malicious JSON before the helper re-execed as root and read it back, getting arbitrary file operations as admin. The fix uses `tempfile::Builder` (random suffix, mode 0600) and `fsync`+`flush` before handing the path to the helper. The `.res` file the helper writes is also explicitly chmod 0600 to keep the (potentially path-revealing) error message from leaking. Two regression tests cover the perms and the non-predictable name. The `tempfile` crate moved from `[dev-dependencies]` to `[dependencies]`.
  - **C2 — `TransferEngine::submit_job` no longer resets the shared policy.** Before, the engine called `self.policy.reset()` on the shared `Arc<dyn TransferPolicy>`; if a second job was enqueued while the first worker was still running (and had already accumulated `AccessDenied` entries), the reset wiped them and the retry-as-admin prompt never fired. The coordinator only runs one worker at a time, so the reset is now done at the start of `TransferWorker::run()`. Regression test: `submit_job_does_not_reset_shared_policy` uses a counting `TransferPolicy` to assert that `submit_job` itself never calls `reset`.
  - **M1 — Compress / Extract now report failures to the policy.** `run_compress` and `run_extract` used `?` to propagate pipeline errors straight to the engine match, bypassing `report_file_failure`. An `AccessDenied` on the destination archive or extract directory no longer triggers the retry-as-admin prompt. A new `report_compress_extract_failure` helper records the failure through the same path Copy / Move / Delete use. On the user side, `handle_permission_answer` now shows a clear `PopupType::Error` with a per-op hint when the user clicks Yes for an operation (Compress / Extract / Rename / CreateLink) that the helper cannot re-run as a sequence of per-file `FsOperation`s, instead of silently doing nothing.
  - **M2 — `apply_command` popup is no longer a silent stub.** A10 left the handler logging a warning and refreshing the panels; the user saw no visible feedback and could reasonably believe their command had been applied to the selected targets. The handler now opens a `PopupType::Error` that echoes the input and tells the user to use the terminal popup or their shell. The input is still logged to `app.log` so power users can recover it.
  - **M3 — `sanitise_entry_name` now rejects Windows drive-prefix and UNC paths.** The leading-`/` and leading-`\\` check on its own is not enough on Windows: a name like `C:/foo` (or `C:\foo`, `C:foo`, `\\server\share\x`, `\\?\C:\foo`) passes that check, the component walker only rejected `ParentDir` and `RootDir`, and `dst_dir.join("C:/...")` is treated as **absolute** by `Path::join` — replacing the base. The result was a Windows zip-slip where a malicious archive could extract to e.g. `C:\Users\victim\.ssh\id_rsa` even though the user thought they were extracting under their chosen destination. The fix adds a `Component::Prefix` arm and a final `is_absolute()` belt-and-suspenders check. Regression test covers `C:/foo`, `C:\foo`, `C:foo`, `\\server\share\file`, and `\\?\C:\foo` on Windows.
  - **M4 — `TransferEndpoint::open_writer` on Local now uses `O_NOFOLLOW` and `O_EXCL`.** `std::fs::File::create` opens with `O_CREAT | O_TRUNC | O_WRONLY`, all of which follow symlinks. `extract_pipeline` in particular always passed `overwrite=true`, bypassing the `path.exists()` short-circuit entirely. If the destination was (or had just become) a symlink, the engine would follow it and overwrite an arbitrary file. The fix uses `OpenOptions::create(true).truncate(true).create_new(!overwrite).custom_flags(O_NOFOLLOW)`; `create_new(true)` is the equivalent of `O_EXCL` and removes the `exists()` → `create()` TOCTOU. `AlreadyExists` is mapped to `EndpointError::AlreadyExists` so the rest of the engine sees the same error kind.
  - **L1 — `secure_erase` (in the elevated helper) closes the `metadata()` → `open()` TOCTOU with `O_NOFOLLOW` and reuses the fd via `try_clone()`.** The user had already approved the operation on the original path, but admin privileges make a successful swap worse than usual.
  - **L2 — ZIP and tar.gz compress pipelines now preserve the source's mode bits.** Both `compress_zip` and `compress_targz` (and their recursive helpers) used to stamp a hard-coded `0o644` onto every entry, so a `0o600` file (e.g. an SSH private key or `~/.netrc`) ended up world-readable in the archive and the tar/zip metadata lied about the file's mode. The fix reads `lstat().mode` (when the endpoint exposes one) and stamps it onto the entry; fallback to `0o644` only when the endpoint cannot surface a mode. Two new `#[cfg(unix)]` regression tests create a `0o600` source, compress it, and assert the entry's stored mode is still `0o600`.
  - **L3 — `RetryRequest::error` is now actually used.** It used to be a `#[allow(dead_code)]`-suppressed field (a direct AGENTS.md violation). The field is now plumbed through to `TransferEvent::PermissionPrompt` as a new `sample_error: Option<String>` so the UI can show a one-line hint next to the file list. The popup type, the keybinding handler, and the renderer are all updated.
  - **L4 — Cleaned stale docstrings** that referenced a non-existent `LoggingPolicy` (in `policy.rs`, `engine.rs`, `worker.rs`) and a non-existent `validate_archive_entry_name` helper (in `pipeline.rs`).
  - **L5 — `PermissionAnswer::Cancel` docstring** no longer claims it "aborts the job if it is still running". The popup is only shown at the end of the job, so there is no running job to abort; `Cancel` is identical to `No` and is kept as a distinct variant for explicit "definitely do nothing" user intent.
  - **L6 — Centralised archive format detection** into `ArchiveFormat::detect_from_path`. The two call sites (`actions/fs_ops/extract.rs` and `input_popup/archive_commands.rs`) used to inline the same extension match and silently fall back to `Zip` for unknown extensions (including `.tar`, which produced a confusing "not a zip file" error). The central helper also recognises `.tar` and `.tar.gz` / `.tgz`; unknown extensions now surface a `PopupType::Error` listing the supported formats.
- **Plugin dispatcher tests no longer race against each other.** `plugin::manager::dispatcher::tests::test_cd_accepts_workspace_path` and `test_cd_rejects_outside_workspace` were failing on parallel test runs because both tests shared the global `PLUGIN_REQ_TX` / `PLUGIN_REQ_RX` mpsc pair: test A's `drain_queue` could pull test B's `PluginRequest::Cd` out of the channel and mutate A's `AppState` with the wrong path. Two commits fix this: the first serialised the two tests on a per-test `OnceLock<Mutex<()>>`; the second refactored the channel so the **receiver is owned by `AppState`** (`state.plugin_req_rx`) and `process_plugin_requests` reads from it directly, while the **sender** is registered into the `PLUGIN_REQ_TX` `OnceLock` by `AppState::new` for the plugin runtime. Each test now gets its own `(Sender, Receiver)` pair via a `fresh_state_with_channel` helper, so the race is impossible by construction — no per-test mutex needed. `PluginManager::init()` is gone (the channel is created in `AppState::new`), and the call to it in `main.rs` is removed. `PLUGIN_REQ_TX` is re-exported at `crate::plugin::manager` for the existing call sites that grab a sender before a state handle exists.
...
- **La pestaña "Plugins instalados" ahora se actualiza al instalar o actualizar, sin tener que cerrar y volver a abrir el modal.** Tras una instalación desde la pestaña de búsqueda o una actualización (`u` / `U`) desde la pestaña de instalados, el `PopupType::PluginMenu` recibía un `InstallFinished` / `PluginActionFinished` que liberaba el lock de "en progreso" pero no refrescaba la lista interna `installed`, así que el usuario tenía que cerrar y reabrir el modal para que apareciera el plugin recién instalado (o la nueva versión). Se agrega la variante `PluginRequest::InstalledPluginsRefreshed(Vec<(String, String, bool, bool, Option<String>)>)`: las tres `tokio::spawn` implicadas (Search install, Installed `u` y `U`) ahora invocan al helper `crate::plugin::updater::fetch_installed_rows_for_refresh` y emiten el request al final; el dispatcher reemplaza el campo `installed` del popup activo con un único `*existing = rows`. El path de apertura del popup (`actions/ui_settings.rs`) se refactoriza para usar el helper compartido `build_installed_rows` y eliminar el bucle duplicado que mantenía. 2 tests nuevos en `plugin::manager::dispatcher::tests` cubren el reemplazo y el no-op cuando no hay popup activo.
