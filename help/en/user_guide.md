...
* **Select Active Plugin (Option 0):** Selects which development plugin is currently targeted for linting, packaging, and local installation. Lists plugins from `plugins_dev_dir` and auto-detects if Panel 1 or Panel 2 contain a plugin directory (contains `manifest.toml`), allowing selection by name, absolute path, or aliases like `panel1`/`panel2` (or `left`/`right`).
...
* **Package (Option 3):** Prepares a local temporary clone of the official `plugin-registry` branch, validates metadata files, copies all plugin assets into the branch, appends/updates its index entry in the master `registry/index.toml` file (metadata only), and embeds the file list with SHA-256 hashes into the manifest copy. It also auto-assigns the MIT license and generates a license file if none is present. It also outputs the absolute path of the local registry cache where the plugin was packaged.
* **Install local development plugin (Option 4):** Copies the active development plugin directly into Pairee's runtime directory and registers it in the lockfile (`plugins.lock`). This makes it appear instantly in the "Installed Plugins" list (Tab 0) so you can execute and test it locally.
* **Submit plugin (GitHub PR) (Option 5):** Prompts for a commit description and initiates the submission process.
